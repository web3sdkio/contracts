...
import { SUPPORTED_CHAIN_ID, Web3sdkioSDK } from "@web3sdkio/sdk";
...
async function main() {
  ...
  if (latest && latest.metadataUri) {
    ...
    for (const [chainId, implementation] of Object.entries(implementations)) {
      ...
      const chainSDK = Web3sdkioSDK.fromPrivateKey(privateKey, chainName);
  ...
  console.log("Release page:", `https://web3sdk.io/${releaser}/${releasedContractName}`);
}
