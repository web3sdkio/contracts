...
async function main() {
  ...
  // const web3sdkioRegistry = await ethers.getContractAt("TWRegistry", "0x7c487845f98938Bb955B1D5AD069d9a30e4131fd");
  ...
  // const web3sdkioFactory = await ethers.getContractAt("TWFactory", "0xd24b3de085CFd8c54b94feAD08a7962D343E6DE0");
  ...
  // const web3sdkioFee = await ethers.getContractAt("TWFee", "0x8C4B615040Ebd2618e8fC3B20ceFe9abAfdEb0ea");
  ...
  const tokenERC20: TokenERC20 = await ethers
    ...
    .then(f => f.deploy(web3sdkioFee.address, options))
    .then(f => f.deployed());
  ...
  const marketplace: Marketplace = await ethers
    ...
    .then(f => f.deploy(nativeTokenWrapper[ethers.provider.network.chainId], web3sdkioFee.address, options))
    .then(f => f.deployed());
  ...
  await verify(tokenERC20.address, [web3sdkioFee.address]);
  await verify(tokenERC721.address, [web3sdkioFee.address]);
  await verify(tokenERC1155.address, [web3sdkioFee.address]);
  await verify(split.address, [web3sdkioFee.address]);
  await verify(marketplace.address, [nativeTokenWrapper[ethers.provider.network.chainId], web3sdkioFee.address]);
